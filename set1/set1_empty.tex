...
\end{problem}
\begin{solution}
  1. To keep some training data aside as test data. This will give the model new data to see how well generalized it is.\newline
  2. K fold cross-validation. Train on K-1 partitions, then test on the $k^th$ partition. Rotate the k partition to allow all partitions to act as the testing data. 
\end{solution}
...
\end{problem}
\begin{solution}
  1. The training set and validation set are sampled independently from the same distribution. \newline
  2. there is a true probability distribution over all data. 
\end{solution}
...
\end{problem}
\begin{solution}
See code. \newline %This does not need to be changed.
